...
set ocv_file "scr/ocv.tcl"
if { [file exists $ocv_file] } {
    fk_msg -type info "Reading OCV from $ocv_file"
    set echo [config_shell -echo false]
    source $ocv_file
    config_shell -echo $echo
} else {
    fk_msg -type info "OCV file $ocv_file is not found"
}
if {$MGC_OCV == "aocv"} {
...
set configs_file "scr/config.tcl"
if { [file exists $configs_file] } {
    fk_msg -type info "Reading configs from $configs_file"
    set echo [config_shell -echo false]
    source $configs_file
    config_shell -echo $echo
} else {
    fk_msg -type info "Configs file $configs_file is not found"
}
...
nrf_utils::set_clock_based_max_transition $MGC_clkBasedMaxTranFactor
...
# store placement locations in database
if { [get_config -name config_auto_learning -param "store_place_locations"] } {
  # default options: type=none
  PLX::UTILS::MLBasedPlaceLocations "store"
}

# store timing violation data
if { [get_config -name config_auto_learning -param "store_setup_violations"] } {
  PLX::UTILS::GetMLSetupViolations
}
