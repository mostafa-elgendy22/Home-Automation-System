...
if {$MGC_libDbPath =="" || ![file exists $MGC_libDbPath]} {
    ...
    # ADVANCED: Read the AOCV files if defined
    if { [info exists MGC_aocv_library] && [array size MGC_aocv_library] > 0} {
       foreach lib_corner [array name MGC_aocv_library] {
	  read_aocv_lib -file $MGC_aocv_library($lib_corner) -name_prefix $lib_corner -corners $lib_corner
       }
    }
    ...
    if { [info exists MGC_disable_SDA_vias_creation] && $MGC_disable_SDA_vias_creation } {
        ...
        config_lib_vias -use_generated true -create true -select true -use_asymmetrical true -use_4cut true -select true
    } else {
...
if { [info exists MGC_floorLoadDefFile] && $MGC_floorLoadDefFile != "" } {
     if { [info exists MGC_KeepDefRegions] && $MGC_KeepDefRegions } {   
     fk_msg -type warning "Reading regions from DEF. Region elements should be checked for accuracy. DEF may assign leaf cells to regions. Region elements should be hierarchical cells to avoid incorrect region association for new cells added during design implementation.\n"
	 read_def $MGC_floorLoadDefFile
     } else {
...
if { $MGC_customNdrFile != "" } {
    fk_msg -type info "Using Custom NDR File : $MGC_customNdrFile"
...
foreach mode $MGC_modes {
    ...
    foreach constraintFile $MGC_importConstraintsFile($mode) {
	if {[file extension $constraintFile] == ".mxdb" } {
	    fk_msg  "Reading Constraints from MXDB for Mode $mode"
	    read_mxdb -skip { def directives library netlist scandef } -file $constraintFile -nitro_mode $mode
	} else {
	    fk_msg  "Reading Constraints from SDC for Mode $mode"
	    read_constraints \
		-modes $mode \
		-file $constraintFile \
		-clock_suffix _${mode}
...
# source customizations the user would like before saving final import db
